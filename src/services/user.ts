...
export async function getUserInfoById({ id }: GetUserInfoByIdParams) {
...
export async function getUserInfoByUsername({
  ...
  const userResumes = await db
    .select()
    .from(resumes)
    .where(eq(resumes.userId, usernameRow.userId))
    .all();

  const userProfile = await db
    .select()
    .from(users)
    .where(eq(users.id, usernameRow.userId))
    .get();
